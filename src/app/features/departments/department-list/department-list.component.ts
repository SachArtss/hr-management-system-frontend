...
export class DepartmentListComponent implements OnInit {
  displayedColumns: string[] = ['name', 'location', 'manager', 'budget', 'employeeCount', 'actions'];
  ...
  applyFilters(): void {
    ...
    this.dataSource.data = this.allDepartments.filter(department => {
      ...
      const matchesSearch = 
        ...
        (department.manager&&department.manager.name && department.manager.name.toLowerCase().includes(searchTerm)) ||
        (department.budget && department.budget.toString().includes(searchTerm));
  ...
  getManagerName(department: Department): string {
    return department.manager&& department.manager.name? department.manager.name: 'Not assigned';
  }

  getEmployeeCount(department: Department): number {
    return department.employees&& department.employees.length? department.employees.length: 0;
  }
